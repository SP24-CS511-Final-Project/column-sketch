...
// This example is copied from https://github.com/apache/arrow-rs/blob/master/parquet/examples/read_with_rowgroup.rs
...
async fn main() -> Result<()> {
  ...
  d.pop();
  let testdata = d.join("parquet-testing/data/alltypes_plain.parquet");
  println!("{:?}", testdata);
  let mut file = File::open(&testdata).await.unwrap();
...
impl InMemoryRowGroup {
  ...
  pub fn build_reader(
    ...
    let levels =
      parquet_to_arrow_field_levels(&self.metadata.schema_descr_ptr(), self.mask.clone(), None)?;
